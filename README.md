...
安装脚本会创建项目自己的 Python 3.11 虚拟环境，不改动系统 Python。依赖来自带 SHA256 的锁文件，构建工具也固定版本，不会在安装时自行升级 pip。ModelScope 轻量包不包含大型模型权重；首次安装会从项目的 GitHub Release 下载 OpenVINO FP16 模型。下载器限制归档字节数与解压范围，并核对归档及模型文件的 SHA256。模型准备完成后，文档审计可以断网运行。
...
当前共有 27 项自动测试，覆盖输入哈希、冲突位置、失败状态、模型校验、下载与解压边界、报告证据和写作规则。
