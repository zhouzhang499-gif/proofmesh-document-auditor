...
- [x] ModelScope 专用 ZIP 小于 5MB。
  ...
  - 最终证据：【待填：文件名、字节数】
- [ ] ZIP 不含 `.venv`、`.build`、运行报告、缓存、凭证、个人绝对路径和大型模型权重。
  - 证据：【待填：排除项检查日志】
- [x] ZIP SHA256 已计算并写入发布记录。
  - 证据：打包脚本同时生成 `dist/proofmesh-modelscope-v0.1.0.zip.sha256`，避免把归档自身哈希写进归档。
- [ ] 英文名称为 `proofmesh-document-auditor`，展示名称为“ProofMesh 文档一致性检查”。
...
- [ ] 最终轻量包不再依赖仓库内的大型模型权重；模型分发方式与 README 一致。
- [ ] 模型来源、固定 revision、文件大小、SHA256 和许可证写入 `model-manifest.json` 或等价发布清单。
  - 当前独立模型包：`dist/proofmesh-openvino-model-v0.1.0.zip`
  - SHA256：`2df1ad03de1359859eb59ea168d770eaebd49634ed82e9a4f3e4d2b7e861a561`
- [ ] 全新用户目录可以完成安装，不读取开发机的 `.venv`、`.build` 或下载缓存。
  - 安装命令：【待填】
  - 安装日志：【待填】
- [ ] 模型完整时正常加载；文件缺失、大小错误或 SHA256 错误时给出可理解的错误。
- [ ] 模型缓存完成后，断网热运行成功。
  - 证据：【待填：断网状态、`run_id`、`model_info.json`】
- [ ] `model_info.json` 记录实际设备和 OpenVINO 版本。
- [ ] 只有经过实测的设备写入文章和页面。当前已有事实仅限 CPU；GPU/NPU 未验证前不得宣传。
- [ ] `THIRD_PARTY_NOTICES.md` 与最终依赖、模型和分发文件一致。
- [ ] Apache-2.0 完整许可证文本随包分发。

## C. 功能与失败路径

- [ ] XLSX、DOCX、PPTX 和带文本层 PDF 均通过端到端检查。
- [ ] 演示包得到与 `ground_truth.json` 一致的结果。
  - 期望：2 个确定性问题、1 个待确认候选。
  - 实际：【待填】
- [ ] 金额、百分比、日期以及同比/环比口径至少各有覆盖样例。
- [ ] 每个确定性问题保留文件路径和原生 locator。
- [ ] 待确认候选与确定性问题分别保存在 `review_candidates.json` 和 `issues.json`。
- [ ] 输入文件处理前后 SHA256 相同；发现变化时停止生成结果。
- [ ] 扫描 PDF 无文本层时标记 `needs_ocr`，运行状态为 `partial`。
- [ ] Excel 公式缺少缓存值时标记 `parsed_with_warnings`，不把公式字符串当成计算结果。
- [ ] 损坏文件标记 `error`，运行状态为 `partial`，不得显示“没有发现问题”。
- [ ] 模型不可用时，报告说明近似候选未执行。
- [ ] UNC、网络路径、符号链接/重解析点越界和输出目录位于输入目录内的请求均被拒绝。
- [ ] 源文档中的指令性文字只作为数据，不改变扫描目录与行为约束。
...
- [ ] README 和报告没有 error 级写作 finding；`style_findings.json` 已回读。
...
- [ ] README 中的安装、审计、状态和查看运行命令均在干净环境逐条执行。
- [ ] README 明确支持格式、扫描 PDF、Excel 公式、CPU 设备和不提供业务裁决等边界。
- [ ] [`modelscope-article.md`](modelscope-article.md) 中所有“待填”已替换为实际证据，或在发布稿中保留为明确的未完成说明。
- [ ] 文章包含场景、第一性原理、公开项目取舍、架构、本地隐私、OpenVINO、评测、Qoder 复现、限制和 Hybrid AI 思考。
...
- [ ] 仓库为公开状态，默认分支可直接访问。
- [ ] `.gitignore` 排除模型缓存、运行报告、虚拟环境和本地凭证。
- [ ] 当前 Git 历史不含 Token、认证键、用户名、个人绝对路径和敏感文档。
- [ ] Release 版本为 `v0.1.0`，包含轻量包、SHA256、安装说明和已知限制。
- [ ] `LICENSE`、`THIRD_PARTY_NOTICES.md`、README 与实际发布内容一致。
- [ ] Release 文件与 ModelScope 提交文件属于同一版本，并用 SHA256 核对。
- [ ] 没有复制许可证不明项目的源码、提示词或模板文本。
  - Release 链接：【待填】
  - 提交 commit：【待填】
