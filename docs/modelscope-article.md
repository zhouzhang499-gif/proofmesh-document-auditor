...
|---|---|---|
| 真实生产力场景 | 检查项目交付文件夹中的跨格式事实漂移 | 四格式原创演示包、25 项自动测试和 100 条事实对评测已完成 |
...
模型与轻量 Skill 分开发布。当前独立模型包为 `proofmesh-openvino-model-v0.1.0.zip`，SHA256 为 `2df1ad03de1359859eb59ea168d770eaebd49634ed82e9a4f3e4d2b7e861a561`。加入图标、评测数据和发布文档后的 ModelScope 轻量包仍小于 1MB；最终归档哈希由打包脚本写入同名 `.sha256` 文件，避免把归档自身哈希写进归档。
...
2026-08-24 的最新本机复验使用 Windows 11、Python 3.11.9 和 OpenVINO 2026.3.0，25 项自动测试全部通过。此前同一环境的演示审计处理了 5 个支持文件，得到 2 个确定性问题和 1 个待确认候选；运行记录中的设备为 CPU，输入文件处理前后哈希一致。这是一轮开发机验证，公开发布前仍需保存正式测试日志，并在干净环境重跑。
...
| 指标 | 结果 | 证据 |
|---|---:|---|
| 自动测试 | 25/25；Windows 11、Python 3.11.9、OpenVINO 2026.3.0 | 【待填：正式测试日志或 CI 链接】 |
