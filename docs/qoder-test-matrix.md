...
执行 Qoder 测试前的代码基线已经通过 25 项自动测试。确定性规则评测覆盖 100 条事实对，Precision、Recall、F1、Accuracy、明确样例正确率、困难样例正确率和定位字段完整率均为 100%。这些结果保存在 `evaluation/results/`，只证明本地规则基线，不能替代下面的 Qoder 路由与交互证据。
